...
async function sendConfirmationEmail(data: { name: string; email: string; date: string; experience: string; locale: 'en' | 'es' }) {
  ...
  const subject = data.locale === 'en'
    ? 'Your Wine & Flow enquiry has been received'
    : 'Hemos recibido tu consulta de Wine & Flow';

  const body = data.locale === 'en'
    ? `Dear ${data.name},\n\nThank you for reaching out. Someone from the Wine & Flow team will be in touch within 24 hours.\n\n— The Wine & Flow Team`
